...
import { useRoqPlatformUserProfiles, useApplicationCount, useDeploymentCount, useBugCount } from 'lib/roq';

import { BugListPage as ShowCaseList } from 'pages/bugs';
...
function DashboardPage() {
  ...
  const deploymentStat = useDeploymentCount();

  const bugStat = useBugCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Deployments',
      result: deploymentStat?.data,
      isLoading: deploymentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bugs',
      result: bugStat?.data,
      isLoading: bugStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`bugs`} passHref legacyBehavior>
                  <Button
